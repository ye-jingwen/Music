...
import { selectLikeSongName,getCollectOfUserId } from '../api/index';
    ...
    methods: {
        ...
        getSong() {
            if (!this.$route.query.keywords) {
                ...
                selectLikeSongName(this.$route.query.keywords).then(res => {
                    if (!res.length) {
                        this.$store.commit('setListOfSongs', []);
        ...
        parseLyric(text) {
            ...
            for (let item of lines) {
                ...
                for (let item1 of time) {
                    ...
                    if (value != '') {
                        result.push([parseInt(t[0], 10) * 60 + parseFloat(t[1]), value]);   //换算成秒
                    }
                }
            }
            //按照第一个元素"时间"排序
