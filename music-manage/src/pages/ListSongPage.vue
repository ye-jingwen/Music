<template>
    <div class="table">
        <div class="crumbs">
            <i class="el-icon-tickets" />歌单歌曲信息
        </div>
        <div class="container">
        ...
        <!-- 歌曲列表显示 -->
        <el-table size="mini" border style="width:100%" height="500px" :data="tableData" @selection-change="handleSelectionChange">
...
export default {
    ...
    methods: {
        ...
        getData() {
            this.tempData = [];
            this.tableData = [];
            selectListSong(this.songListId).then((res) => {
                for (let item of res) {
        ...
        getSongId() {
            ...
            selectBySongName(songOfName).then((res) => {
                _this.insertSong(res[0].id);
            });
        },
        //添加歌曲
        insertSong(songId) {
